...
\begin{minipage}[t]{\textwidth}
\begin{itemize}
\item<3-> $F : 2^V \to \mathbb{R}$
\end{itemize}
\end{minipage}
&
\begin{minipage}[t]{\textwidth}
\begin{itemize}
\item<6-> Set of factors $\phi_i : \mathcal{X}_i \to \mathbb{R}$
...
\end{itemize}
\end{minipage} & \uncover<2->{\color{col1}powerset of $V$}\\[1.5em]
\begin{minipage}{0.45\textwidth}
\begin{itemize}
\item Transition matrix $P$
\end{itemize}
\end{minipage} & \uncover<3->{\color{col1}Gibbs sampler}\\[1.5em]
\begin{minipage}{0.45\textwidth}
\begin{itemize}
\item Stationary distribution $\pi$
\end{itemize}
\end{minipage} & \uncover<4->{\color{col1}PSM distribution}
\end{tabular}
...
\only<11-12>{We bound the maximum congestion of a PSM using $\zeta_f$}
...
\only<8-11>{\includegraphics[width=3.6in]{figures/cp_easy.pdf}}%
